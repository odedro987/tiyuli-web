...
export namespace tiyuli.expense {
    export class NewExpenseRequest extends pb_1.Message {
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.types.length)
                writer.writeRepeatedString(3, this.types);
            if (this.has_payment_date)
                writer.writeMessage(4, this.payment_date, () => this.payment_date.serialize(writer));
            if (this.currency_code.length)
                writer.writeString(6, this.currency_code);
            if (this.amount != 0)
                writer.writeFloat(7, this.amount);
            if (!w)
                return writer.getResultBuffer();
        }
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): NewExpenseRequest {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new NewExpenseRequest();
    ...
    export class NewExpenseResponse extends pb_1.Message {
        ...
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): NewExpenseResponse {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new NewExpenseResponse();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
